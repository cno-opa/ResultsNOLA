...
#### Code other relevant Law and Exec columns to dates
...
#### Filter out rows generated by ECMS approval path errors
Errors<-filter(contractPOapproval,Stage=="Error")
contractPOapproval<-filter(contractPOapproval,Stage!="Error")

#### Filter for contracts with an approval date, to ensure only one instance of a given stage is left for each contract, drop Approver column
...
contractPOapproval<-select(contractPOapproval,PO,AltID,PO:LegalReview,DepAttorney,CAO,SentVendor,ECMS_BackFromVendor,ExecutiveSignature)
...
contracts<-select(contracts,PO,Req,AltID,Dept:Vendor,Description:LawStatus,POStatus,ReqStatus,Requestor:Purchaser
...
Open_closedplot<-ggplot(summary_melt,aes(x=factor(Qtr),y=value,fill=variable))+
  ...
  theme(legend.title=element_blank(),plot.title=element_text(size=13,face="bold",vjust=1))
...
#### Create file with both days to execute and days per stage charts
multiplot(Execution,Stage_plot)
ggsave("./ReqtoCheckSTAT/Query Files/Slides/Contract POs/Execute Slide (Multi).png")
...
Vendor$Dept2<-ifelse(Vendor$Dept=="AVIATION","Aviation",
                     ifelse(Vendor$Dept=="CAO","CAO",
                            ifelse(Vendor$Dept=="CAPITAL PROJECT","CPA",
                                ifelse(Vendor$Dept=="DISASTER CDBG UNIT","CDBG",
                                   ifelse(Vendor$Dept=="FIRE DEPARTMENT","NOFD",
                                          ifelse(Vendor$Dept=="HEALTH DEPARTMENT","Health",
                                                 ifelse(Vendor$Dept=="HOMELAND SECURITY","Homeland",
                                                        ifelse(Vendor$Dept=="Homeland Security Grant Approval","Homeland Grant",
                                                            ifelse(Vendor$Dept=="Human Resources","HR",
                                                               ifelse(Vendor$Dept=="LAW","Law",
                                                                      ifelse(Vendor$Dept=="MAYOR","Mayor",
                                                                          ifelse(Vendor$Dept=="MOSQUITO CONTROL BOARD","MTCB",
                                                                          ifelse(Vendor$Dept=="MUNICIPAL YACHT HARBOR MANAGEMENT CORPORATION(MYHMC)","MYHMC",
                                                                      ifelse(Vendor$Dept=="NEIGHBORHOOD STABILIZATION","NS",
                                                                  ifelse(Vendor$Dept=="NEW ORLEANS BUILDING CORPORATION","NOBC",
                                                              ifelse(Vendor$Dept=="NORD RECREATION DEPARTMENT","NORDC",
                                                            ifelse(Vendor$Dept=="OFFICE OF CRIMINAL JUSTICE COORDINATOR","OCJC",
                                                         ifelse(Vendor$Dept=="OFFICE OF INSPECTOR GENERAL","OIG",
                                                     ifelse(Vendor$Dept=="OFFICE OF TECHNOLOGY & INNOVATION","ITI",
                                                  ifelse(Vendor$Dept=="POLICE DEPARTMENT","NOPD",
                                                ifelse(Vendor$Dept=="PARKS & PARKWAYS","PPW",
                                              ifelse(Vendor$Dept=="PROPERTY MANAGEMENT", "Prop Mgmt",
                                          ifelse(Vendor$Dept=="Public Library","Library",
                                      ifelse(Vendor$Dept=="PUBLIC WORKS","DPW",NA))))))))))))))))))))))))
Vendor_plot<-ggplot(Vendor,aes(x=Count,y=Age))+
  ...
  geom_text(aes(label=Dept2,size=7))+
  theme(plot.title=element_text(size=13,face="bold",vjust=1))
print(Vendor_plot)
...
write.csv(LawKPI,"O:/Projects/STAT KPIs/ReqtoCheckSTATLaw KPI.csv")
